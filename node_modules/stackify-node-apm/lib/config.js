...
function Config (platform) {
  ...
  this.setPlatform(common.PLATFORM)
...
Config.prototype._setupConfiguration = function (fileLocation, fileName) {
  ...
  if (userConfiguration && !Util.isUndefined(userConfiguration.debug) && Util.isBoolean(userConfiguration.debug)) {
  ...
  } else if (userConfiguration && !Util.isUndefined(userConfiguration.rum_enabled) && Util.isBoolean(userConfiguration.rum_enabled)) {
  ...
  } else if (userConfiguration && !Util.isUndefined(userConfiguration.prefix_enabled) && Util.isBoolean(userConfiguration.prefix_enabled)) {
  ...
  } else if (userConfiguration && !Util.isUndefined(userConfiguration.queue_enabled) && Util.isBoolean(userConfiguration.queue_enabled)) {
  ...
  let promiseEnabled = !Util.isUndefined(process.env['STACKIFY_PROMISE_ENABLED']) && process.env['STACKIFY_PROMISE_ENABLED']
    ? process.env['STACKIFY_PROMISE_ENABLED'] : null

  if (promiseEnabled) {
    this.promiseEnabled = Boolean(JSON.parse(promiseEnabled))
    Logger.debug('Config._setupConfiguration() - Using STACKIFY_PROMISE_ENABLED environment variable')
  } else if (userConfiguration && !Util.isUndefined(userConfiguration.promise_enabled) && Util.isBoolean(userConfiguration.promise_enabled)) {
    this.promiseEnabled = userConfiguration.promise_enabled
  }
  if (userConfiguration) userConfiguration.promise_enabled = this.promiseEnabled
  this.promiseEnabled = Boolean(JSON.parse(this.promiseEnabled))
...
Config.prototype._validate_config = function (userConfiguration) {
  ...
  if (userConfiguration && !Util.isUndefined(userConfiguration.promise_enabled) && Util.isBoolean(userConfiguration.promise_enabled) === false) {
    Logger.error('Config._setupConfiguration(): promise_enabled must be a Boolean type.')
  }
}
...
/**
 * Check if promise is enabled
 *
 * @returns {boolean}
 */
Config.prototype.isPromiseEnabled = function isPromiseEnabled () {
  return this.promiseEnabled
}

/**
 * Set promise enabled
 * @param {boolean} isPromiseEnabled
 *
 * @returns {void}
 */
Config.prototype.setPromiseEnabled = function isDebugEnabled (isPromiseEnabled) {
  if (!Util.isBoolean(isPromiseEnabled)) {
    return
  }
  this.promiseEnabled = isPromiseEnabled
}
