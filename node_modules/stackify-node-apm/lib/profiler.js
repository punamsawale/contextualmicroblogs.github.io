...
const CORE_MODULES_INSTRUMENTATION = {
  'serverless-http/lib/framework/get-framework': path.join('serverless-http', 'framework', 'get-framework'),
  'serverless-http/lib/finish': path.join('serverless-http', 'finish')
}

CORE_MODULES_INSTRUMENTATION[path.join(process.cwd(), 'node_modules', 'serverless-http/lib/framework/get-framework')] = path.join('serverless-http', 'framework', 'get-framework')
CORE_MODULES_INSTRUMENTATION[path.join(process.cwd(), 'node_modules', 'serverless-http/lib/finish')] = path.join('serverless-http', 'finish')
...
function wrapModuleRequire (tracer, profiler) {
   ...
   *   for instrumentation
   */
  shimmer.wrap(Module.prototype, 'require', function wrapRequire (original, name) {
    return function wrappedRequire (id) {
      let newObject = {
        name: ''
      }
      resolvedNameObject = newObject
      let result = wrapExport(original.apply(this, arguments), id, newObject)
      return result
    }
  })
...
function startInstrumentation (tracer, profiler) {
  ...
  addConfigCoreInstrumentations(profiler)
...
function addConfigCoreInstrumentations (profiler) {
  // Group by module name
  Object.keys(CORE_MODULES_INSTRUMENTATION).forEach(function objectEach (value, index) {
    let moduleName = value
    let instrumentationPath = CORE_MODULES_INSTRUMENTATION[value]

    let instrumentationFilePath = path.join(__dirname, 'modules', instrumentationPath + '.js')
    let instrumentationFullFilePath = path.resolve(instrumentationFilePath)

    if (!fs.existsSync(instrumentationFilePath)) {
      Logger.warn(
        'Profiler: addConfigCoreInstrumentations - Core - Instrumentation file does not exist - File: %s',
        instrumentationFullFilePath
      )
      return
    }

    let requireFunction = require(instrumentationFilePath)
    if (!Util.isFunction(requireFunction)) {
      Logger.warn(
        'Profiler: addConfigCoreInstrumentations - Failed to add instrumentation - Error: imported file is not a function - File: %s',
        instrumentationFullFilePath
      )
      return
    }
